...
class Cartrepo {
  ...
  Future<Either<Failure, UpdateCartItemModel>> UpdateItemCart(RequestUpdateItemModel request) async {
    try {
      final response = await apiConsumer.put("api/cart/update-item", request.toJson());
      if (response == null || response is! Map<String, dynamic>) {
        return Left(servivefailure("Invalid server response"));
      }
